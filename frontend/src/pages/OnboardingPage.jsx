...
import { complete_onBoadring } from "../lib/Api";
import {
  CameraIcon,
  ShuffleIcon,
  LoaderIcon,
...
function OnboardingPage() {
  ...
  const { mutate: onBoarding_Mutation, isPending } = useMutation({
    mutationFn: complete_onBoadring,
    onSuccess: () => {
      toast.success("Onboarding completed successfully!");
      // Invalidate the auth query to trigger a refetch
      queryClient.invalidateQueries({ queryKey: ["authUser"] });
      navigate("/"); // programmatic navigation after state is updated
    },
    onError: (error) => {
      const errorMessage =
  ...
  const handleSubmitOnboarding = (e) => {
    e.preventDefault();
    onBoarding_Mutation(formState);
  };

  // handler to generate random avatar
